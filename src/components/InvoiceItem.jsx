...
const InvoiceItem = ({ id, name, qty, price, onDeleteItem, onEdtiItem }) => {
  const deleteItemHandler = () => onDeleteItem(id);

  const renderInvoiceField = (fieldName, value, type, min, step = undefined, className = '') => (
    <InvoiceField
      onEditItem={onEdtiItem}
      cellData={{
        placeholder: fieldName === 'name' ? 'Item name' : undefined,
        type,
        min,
        step,
        name: fieldName,
  ...
  return (
    <tr>
      <td className="w-full">
        {renderInvoiceField('name', name, 'text')}
      </td>
      <td className="min-w-[65px] md:min-w-[80px]">
        {renderInvoiceField('qty', qty, 'number', '1')}
      </td>
      <td className="relative min-w-[100px] md:min-w-[150px]">
        ...
        {renderInvoiceField('price', price, 'number', '0.0', '0.0', 'text-right')}
      </td>
